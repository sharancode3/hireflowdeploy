...
begin
  ...
  -- Allow reopening shortlisted applications back to applied.
  if old_status = 'SHORTLISTED' and next_status in ('APPLIED', 'INTERVIEW_SCHEDULED', 'REJECTED') then
    return true;
  end if;

  -- Allow cancelling interviews back to applied/shortlisted.
  if old_status = 'INTERVIEW_SCHEDULED' and next_status in ('APPLIED', 'SHORTLISTED', 'OFFERED', 'REJECTED') then
    return true;
  end if;
